...
class zcq_pro_type extends MY_Controller
{
    ...
    function index()
    {
        ...
        $orderby["(endtime-starttime)"] = "desc";
        if($search_val["title"]!=""){
    ...
    function add(){
        ...
        if(is_array($post)){
            ...
            $this->parent_showmessage(
                1
                ,( $model["check_status"]=="99"?"您的申请已保存，在【我的申请】里":"提交成功，等待管理员审核"),
                $this->data["ls"],
                3,
                'showmessage_logout'
            );
            ...
            if($id>0){
                ...
                $this->data["status"] = $this->zcq_pro_type->getstatus($model["starttime"],$model["endtime"]);
                ...
                $this->parent_showmessage(
                    1
                    ,"无资料",
                    site_url("adminx/zcq_pro_type/index"),
                    3,
                    'showmessage_logout'
                );
                exit();
            }
            //检查是否申请过,未审或通过，都不能再申请
            $chk_count = $this->zcq_pro_shenqing->count( "create_userid='".$userid."' and type_id='".$id."' and (check_status='0' or check_status='10' or check_status='99') and isdel='0' ");
            //读出项目需要的附件
            $fujian =  $this->zcq_pro_type_fujian->getlist("del_sysuserid='0' and type_id='".$id."'","orderby asc");
 ...
 */
    ...
    {
        $post = $this->input->post();
        if (isset($post["fjpath"])) {
            if (is_file($post["fjpath"])) {
                @unlink($post["fjpath"]);
            }
        }
    }
