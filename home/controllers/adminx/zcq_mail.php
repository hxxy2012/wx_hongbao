...
class zcq_mail extends MY_Controller {
	...
	function index(){
		...
		$search["t1.receive_userid"] = "='".$this->data["sess"]["userid"]."'";
	...
	function add(){
		...
		if(is_array($post)){
            ...
            $userid = isset($this->data["sess"]["userid"])?$this->data["sess"]["userid"]:0;
            $adminid = 0;
            ...
            if(isset($post["ishelp"])){
                ...
                $ls = empty($post["ls"]) ? site_url("adminx/zcq_mail/add") : $post["ls"];
                //showmessage("发送成功", $ls, 3, 1);
                $this->parent_showmessage(
                    1
                    ,"发送成功",
                    site_url("adminx/zcq_mail/add"),
                    3,
                    'showmessage_logout'
                );
                exit();
            ...
            //常发送的
            $qiye = $this->zcq_mail->getlist_huiyuan_orderby("t1.create_userid='".$this->data["sess"]["userid"]."' and t1.receive_userid>0 and t2.isdel='0' and usertype='45063'");
            $jigou = $this->zcq_mail->getlist_huiyuan_orderby("t1.create_userid='".$this->data["sess"]["userid"]."' and t1.receive_userid>0 and t2.isdel='0' and usertype='45064'");
            $sysuser_list = $this->zcq_mail->getlist_sys_orderby("t1.create_userid='".$this->data["sess"]["userid"]."' and t1.receive_sysuserid>0");

			if($this->data["ls"]==""){
				$this->data["ls"]=site_url("zcq_mail/add");
			}
            $this->data["qiye"] = $qiye;
	...
	function selmember(){
        ...
        if(is_array($post)){
            ...
            echo "parent.layer.msg(\"成功添加\",{time:1000});";
            echo "parent.closefj();";
            die("</script>");
        }
        else {
            ...
            $search['status']="=1";//只能对正常用户发邮件
            $search['checkstatus']="=1";//只能对审核通过用户发邮件
            $pageindex = 1;
            $search['exp'] = " t1.uid!={$userid}";
	...
	function del(){
		...
		if($ids!=""){
			foreach($arr as $v){
			    //检查是否能删除
                //echo $this->zcq_mail->count("id='".$v."' and receive_userid='".$this->data["sess"]["userid"]."'");
                if($this->zcq_mail->count("id='".$v."' and receive_userid='".$this->data["sess"]["userid"]."'")>0){
                    $this->zcq_mail->del($v);
                }
	...
	function myview(){
        ...
        if($id>0){
            ...
            if( $model["receive_userid"]==$this->data["sess"]["userid"] ){
                if($model["isread"]=="0") {
                    $model["isread"] = "1";
