<?php
class m_zcq_pro_type extends M_common {
	private $table = "zcq_pro_type";
	private $tablename = "项目类型";
	function m_zcq_pro_type(){
		parent::__construct();	
	...
	function GetInfoList($pageindex,$pagesize,$search,$orderby){
		...
		foreach($list as $k=>$v){
		    $tmp = $this->getstatus($v["starttime"],$v["endtime"]);
		    $list[$k]["status_name"] = $tmp[0];
            $list[$k]["status_color"] = $tmp[1];
            $list[$k]["status_val"] = $tmp[2];
        }
		$data = array(
				"pager"=>$page_string,
				"list"=>$list
     ...
     * @param $endtime   结束时间
     * @return array     返回状态及颜色 如：未开始，已结束，进行中
     */
