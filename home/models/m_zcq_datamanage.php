...
class m_zcq_datamanage extends M_common
{
    ...
    public function getList($pageindex, $pagesize, $search)//$admintype
    {
        ...
        $sql = "select t1.*, t2.uid from {$this->table} t1 LEFT JOIN {$this->usertable} t2 ON ( t1.userid = t2.uid ) WHERE {$where} limit {$limit}";
     ...
     * @internal param $id
     */
    public function getList_qy($pageindex, $pagesize, $search, $orderby = array())//$admintype
    {
        //分页
        $this->load->library("common_page");
    ...
    function update($model)
    {
        ...
        write_action_log(
            ...
            get_client_ip(),
            ($arr['affect_num'] >= 1 ? 1 : 0),
            "设置用户阅读走出去数据：" . $model["id"] . ($arr['affect_num'] >= 1 ? "成功" : "失败"));
