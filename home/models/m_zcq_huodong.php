...
class M_zcq_huodong extends M_common {
    ...
    function GetInfoList($pageindex, $pagesize, $search, $orderby) {
        ...
        foreach ($search as $k => $v) {
            ...
            } else if ($k == 'userid') {//用户id
                $where .= " and t2.userid='$v'";
                $leftjoin .= "left join zcq_huodong_baoming t2 on t1.id=t2.huodong_id ";
            } else {
                $where .= " and t1.$k='$v'";
            }
        }
        ...
        left join zcq_huodong_baoming t2 on t1.id=t2.huodong_id 
		 {$where}";
