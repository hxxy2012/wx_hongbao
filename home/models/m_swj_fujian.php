<?php
class M_swj_fujian extends M_common {
	private $table = "swj_fujian";
	private $tablename = "通用附件";
	function M_swj_fujian(){
		parent::__construct();	
	...
	function GetUrl($id){
		if($id>0){
			...
			return isset($model["filesrc"])?$model["filesrc"]:"";
		}
		else{
			return "";	
		}
	...
	//更新附件表中的备注字段，提交保存后“临时”清空,$arr_id:清空的数组id
	function update_beizhu($arr_id) {

		$str_id = @implode(',', $arr_id);//组合为以逗号分隔的字符串
		if ($str_id != '') {
			$sql = "update ".$this->table." set beizhu='' where id in($str_id)";
			return  $this->M_common->update_data($sql);
		}
		return '';
	}
	//获取多个文件数组,$str_id:id字符串
	function getModelList($str_id) {
		if ($str_id) {
			$sql = "select * from ".$this->table." where id in ($str_id)";
