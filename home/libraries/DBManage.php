...
//
//
//$db = new DBManage('localhost', 'root', 'root', 'rwfp', 'utf8');
//
////导出
// $list_tables = $db->list_tables('rwfp');
// // print_r($list_tables);exit;
...
//        $dir = $dir ? $dir : 'backup/' . date('Ymd', time()).'/';
        $dir = $dir ? $dir : 'backup/' . date('Ym', time()) . '/' . date('Ymd', time()) . '/';
        $size = $size ? $size : 2000;
        $sql = '';
        // 只备份某个表
        if (!empty($tablename)) {
