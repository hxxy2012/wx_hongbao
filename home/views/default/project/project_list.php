...
<div class="form-inline definewidth m20" > 
  ...
  <select id="status" name="status">
    ...
    <a  class="btn btn-primary page-action" data-id='ckwqxm' id="ckwqxm" title="查看往期项目"
     data-href='<?php echo site_url("swj_project/index")."?status=2";?>' href="#">查看往期项目</a>&nbsp;&nbsp; 
    <a  class="btn btn-primary page-action" data-id='wdsb3' id="wdsb3" title="我的申报"
     data-href='<?php echo site_url("Swj_mime_project/index");?>' href="#">我的申报</a>
     <a class="btn btn-warning" href="/other/help/sb.htm" target="_blank">帮助</a>
</div>
...
function common_request(page){
  var url="<?php echo site_url("Swj_project/index");?>?inajax=1";
  var data_ = {
    'page':page,
    'time':<?php echo time();?>,
    'action':'ajax_data',
     ...
     //  async:false,
      ...
      if(msg.resultcode<0){
        ...
        for(var i in list){
          ...
          shtml+='<td>'+list[i]['timepart']+'</td>';
          if (list[i]['status'] == '进行中') {
            shtml+='<td><a title="申报" href="<?php echo site_url('Swj_project/shenbao');?>?id='+list[i].id+'">申报</a>&nbsp;&nbsp;</td>';
          } else {
            shtml+='<td></td>';
          }
          /*else if (list[i]['status'] == '已申报') {
            shtml+='<td>已申报</td>';
          } else if (list[i]['status'] == '已过期') {
            shtml+='<td>已过期</td>';
          } else {
            shtml+='<td>未开始</td>';
          }*/
...
function del_sp(id, object) {
  if (confirm('您确定要删除该备案吗？')) {
    window.location.href = "<?php echo site_url('Swj_project/del');?>?id=" + id;
  }
}
//批量删除
function pldel() {
  ...
  if (confirm('您确定要删除选中的备案吗？')) {
    window.location.href = "<?php echo site_url('Swj_project/del');?>?id=" + ids;
  }
}
//批量审核
function plshenhe() {
  ...
  if (confirm('您确定要审核选中的备案吗？')) {
    window.location.href = "<?php echo site_url('Swj_project/shenhe');?>?pl_audit_idea="+pl_audit_idea+"&audit_status="+audit_status+"&id=" + ids;
  }
}
//获取所有选中的复选框的id,以逗号分割
function getIds() {
