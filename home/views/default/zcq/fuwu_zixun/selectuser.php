...
<body class="definewidth" style="padding-left:10px;">
    ...
    <form method="get" class="form-inline">
        ...
        <label id="lab_servertype">
            ...
            foreach ($server_type_list as $item) {
                ...
                echo "<label style='margin-right: 5px'><input type='checkbox' name='server_type[]' value='{$item['id']}' {$selected}>{$item['name']}</label>";
            }
            ?>
        </label>
...
<table class="pc_member_background_list table table-bordered"
       style="width: 100%;margin-left: 0;float: none;margin-top: -25px">
    <thead>
    <tr>
        <!--        <th width='50'>编号</th>-->
        <th style="width: 150px;">类型</th>
        <th style="width: 250px;">简称</th>
        <!--        <th>联系人/电话</th>-->
        <!--        <th width='60'></th>-->
    </tr>
    </thead>
    ...
    foreach ($list as $item) {
        ...
        foreach ($server_type_list as $each) {
            $pos = strpos($item["server_type"], $each["id"]);
            if ($pos !== false && $pos >= 0) {
                $str .= $each['name'] . "/";
            }
        }
        ...
        echo "<td><span style='color: #3a4bfb'>[{$str}]</span></td>";
        //全称
...
<div style="margin-bottom: 30px;text-align: center">
    ...
    <button onclick="top.closeuser();" class="mybtn">关闭窗口</button>
</div>
