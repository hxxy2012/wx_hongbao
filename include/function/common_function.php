...
if(!function_exists("verify_double")){
	function verify_double($id=null) {
		if (!$id) { 
			return 0;
...
if(!function_exists("auth_code")){
	...
	/*
		...
		当此值为 0 时，则不产生随机密钥
	*/
	//$string = str_replace("8888","+",$string);
	//$string = str_replace("9999","/",$string);
	$ckey_length = 4;
	...
	if($operation == 'DECODE') {
		...
		//$tmp = str_replace("+", "8888", $tmp);
		//$tmp = str_replace("/", "9999", $tmp);
		//return $tmp;
	}
}	
}
...
/*
 *发送短信成功,插入到记录表中
 *参　　数：$content:短信内容，$suid:发送用户id, $stype：发送用户类型
 			,$phone:发送到的手机号码多个以逗号分割,$ruid:接收用户id, $rtype：接收用户类型
 *返 回 值：返回检测结果，ture or false 
 */

if( !function_exists("send_message") ){
	function send_message($content, $suid, $stype, $ruid, $rtype, $phone) {
		if (do_sms($content, $phone)) {//发送短信成功
			//插入到发短信记录表中
...
if( !function_exists("do_sms") ){
	function do_sms($content, $phone) {
		return true;
	}
}
?>