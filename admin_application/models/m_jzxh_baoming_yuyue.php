<?php
class M_jzxh_baoming_yuyue extends M_common {
	private $table = "jzxh_baoming_yuyue";
	function M_jzxh_baoming_yuyue(){
		parent::__construct();	
	...
	function add($model){
		...
		"添加报名预约：".login_name()."|管理员ID=" . admin_id() . ($arr['affect_num']>=1?"成功":"失败"));
	}	
	...
	function update($model){	 
		...
		get_client_ip(),
		($arr['affect_num']>=1?1:0),
		"更新报名预约：".login_name()."|管理员ID=" . admin_id() . ($arr['affect_num']>=1?"成功":"失败"));						
	}
	...
	function GetInfoList($pageindex,$pagesize,$search,$orderby){
		...
		foreach($search as $k=>$v){
			if($k=="linkman"){
				$where .= " and ( t1.".$k." like '%".$v."%' or t1.tel like '%".$v."%' or t2.username like '%".$v."%' or t3.name like '%".$v."%') ";
			}	
			else{		
				$where .= " and ".$k."".$v;
			}
