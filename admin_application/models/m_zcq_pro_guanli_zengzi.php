 ...
 */
class m_zcq_pro_guanli_zengzi extends M_common
{
    //增资表
    public $table = "zcq_pro_guanli_zengzi";
    //管理表
    public $guanlitable = "zcq_pro_guanli";
     ...
     * @return array
     */
    public function getList($pro_id,$pageindex, $pagesize, $search)//$admintype
    {
        //分页
        ...
        $where = " t1.isdel = '0' and t1.pro_guanli_id={$pro_id} ";

        foreach ($search as $k => $v) {
            if ($k == "name") {
                $where .= " and ( t1.companyname like '%{$v}%' or t1.companyname2 like '%{$v}%' )";
            } else if ($k == "zengzi_date_start") {
                $time = strtotime($v);
                $where .= " and t1.zengzi_date >= {$time}";
            } else if ($k == "zengzi_date_end") {
                $time = strtotime($v);
                $where .= " and t1.zengzi_date <= {$time}";
            }
