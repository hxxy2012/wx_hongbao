...
class M_common_category_data extends M_common {
	...
	function GetModelList(){
		return $this->M_common->querylist("select * from ".$this->table." where pid=0 order by disorder asc");
	}
	...
	function getlist($where){
		return $this->M_common->querylist("select * from ".$this->table." where $where order by disorder asc");
	}	
     ...
     */
	function GetSheng(){
		return $this->M_common->querylist("select * from ".$this->table." where typeid=6 and pid=0 order by disorder asc");
	}


	function GetShi($pid){
		return $this->M_common->querylist("select * from ".$this->table." where typeid=6 and pid=$pid order by disorder asc");
	}
	
	function GetListFromTypeid($typeid){
		return $this->M_common->querylist("select * from ".$this->table." where typeid=$typeid order by disorder asc");
	}	
