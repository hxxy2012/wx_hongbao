...
<div class="row">
   <div class="control-group span9">
         <label class="control-label">选择题目：</label>
            <div class="controls">
<span id="dati_title"><?php echo $dati_model!=""?$dati_model["title"]:"";?></span>
<input type="hidden" name="zzb_dy_dati_id" id="zzb_dy_dati_id" value="<?php echo $dati_model!=""?$dati_model["id"]:"";?>"/>            
...
function(){
	parent.my_confirm(
			"是否删除缩略图?",
			"<?php echo site_url("Website_category/delinfothumb");?>?id="+$("#id").val(),
			"<?php echo site_url("dangyuandati/edit");?>?id=<?php echo $main_model["id"];?>"
	);		
});
/*
	...
	var h = 400;
	 BUI.use('bui/overlay',function(Overlay){
         var dialog2 = new Overlay.Dialog({
           ...
           height:h,
           //配置文本
           bodyContent:'<iframe src="<?php echo site_url("dangyuandati/seltiku");?>" width="'+(w-10)+'"  height="'+(h-60)+'" frameborder="0" marginwidth="0" marginheight="0"  ></iframe>',
           buttons:[
                    {
                      text:'确定',
...
function selone(title,id){
	$("#zzb_dy_dati_id").val(id);
	$("#dati_title").html(title);
}
