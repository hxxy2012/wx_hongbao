    ...
    <table cellpadding="10">
        <tr>
            <td>
                ...
                <select id="ad_type">
                    ...
                    if (isset($list) && $list) {
                        foreach ($list as $k => $v) {
                            ...
                            ?>
                            <option
                                value="<?php echo isset($v['id']) ? $v['id'] : ''; ?>"> <?php echo isset($v['typename']) ? $v['typename'] : ''; ?></option>
                            <?php
                        }
                    }
                    ?>
                ...
                <a class="btn btn-success" id="addnew" href="javascript:void(0)" onclick="add_ad()">添加广告<span
        ...
        <tr id="more_search" style="display:none">
            <td>
                ...
                开始日期:
                <!--限制最大时间maxDate:'%y-%M-%d'-->
                <input id="beginDate" class="Wdate" type="text" readonly=""
                       onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true,readOnly:true})"
                       value="" placeholder="" name="">
                ~~~ 结束时间：
                <input id="enddate" class="Wdate" type="text" readonly=""
                       onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true,readOnly:true})"
                       value="" placeholder="" name="">
...
<script>
                ...
                if (msg.resultcode < 0) {
                    ...
                    if (list.length > 0) {
                        for (var i in list) {
                            ...
                            shtml += '<td><a href="<?php echo site_url("ad/edit"); ?>?id=' + list[i].id + '" class="page-action icon-edit" title="编辑广告' + list[i]['name'] + '"></a>&nbsp;&nbsp;<!--<a href="<?php echo site_url("news/index"); ?>?id=' + list[i].id + '&action=preview">查看</a>--></td>';
                            shtml += '</tr>';
                        }
                        $("#result_").html(shtml);
    ...
    $(document).ready(function () {
        //设置当前的页面标题
        top.topManager && top.topManager.setPageTitle("广告列表");

        //编辑打开新窗口
        //动态绑定事件
        $("#result_").on("click", "a.page-action", {abc: 1}, function (event) {
//        var $this = $(this);
//        //console.log(event.data);//{abc:1}
//        top.topManager.openPage({
//            id: 'edit_ad_',
//            href: $this.attr("href"),
