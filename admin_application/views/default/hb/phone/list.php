...
<table class="table table-bordered table-hover  m10">
  ...
  <tbody id="result_">
           ...
           <?php
            if (count($list)<=0) {
            	echo "<tr><td style='text-align:center;' align='center' colspan='4'>暂无内容</td></tr>";
            }
            foreach($list as $v){
            	...
            	echo "<td>";
				echo "<a class='page-action icon-edit' data-href='".site_url("hb_phone/edit")."?backurl=".(urlencode(get_url()))."&id=".$v["id"]."' href=\"#\" data-id='open_hb_edit_".$v["id"]."' id='open_hb_edit_".$v["id"]."' title=\"编辑".$v["tel"]."的手机号\"></a>&nbsp;";
				echo "</td>";                          	            	             	
            	echo "</tr>";
            	echo "\n";
            }
...
function godel(){
	...
	if(ids==""){		
		...
		var ajax_url = "<?php echo site_url("hb_phone/del");?>?idlist="+$("#selid").val();
		//var url = "<?php echo $_SERVER['REQUEST_URI'];?>";
		var url = "<?php echo base_url();?>gl.php/hb_phone/index";
		/*parent.parent.my_confirm(
				"确认删除选中红包？",
				ajax_url,
				url);*/
		BUI.Message.Confirm("确认删除选中手机号？",function(){
			$.ajax({
		        type: "get",
		        url: ajax_url,
		        data: '',
...
function chkdel(){
		var ajax_url = "<?php echo site_url("hb_phone/delcheck");?>?idlist="+$("#selid").val();
		i=0;
		name = "";
		$.ajax({
				url:ajax_url,
