...
<div class="form-inline definewidth m20">
    ...
    <form method="get">
        ...
        <input type="text" name="search_name"
               ...
               placeholder="企业名称/项目名称/拟投资国家/联系人/手机"
        ...
        <select name="search_town">
            ...
            foreach ($town as $item) {
                $selected = $search_val['town'] == $item['id'] ? "selected" : "";
        ...
        <a id="add_new" class='btn btn-info' data-href='<?php echo site_url("zcq_zhongdian_pro/add"); ?>' data-id='354'
           title='添加新的项目'>新增项目</a>
...
<table class="table table-bordered table-hover  m10">
    ...
    foreach ($list as $item) {
        ...
        echo "<td>" . $item["id"] . "</td>";
        //所属镇区
        echo "<td>{$item['town_name']}</td>";
        //企业名称
        echo "<td>{$item["companyname"]}</td>";
        //项目名称
        echo "<td>{$item["pro_name"]}</td>";
        //联系人/手机
        echo "<td>{$item["linkman"]}/{$item["tel"]}</td>";
        //新增时间
        $time = date("Y-m-d", $item['createtime']);
        echo "<td>{$time}</td>";
        //修改按钮
        echo "<td>";
        $view_url = $url_view . "?id={$item["id"]}";
        echo "<a class='page-action icon-list-alt' data-href='{$view_url}' data-id='userlist_{$item["id"]}' title='查看重点项目{$item["id"]}'></a>";

        $edit_url = $url_edit . "?id={$item["id"]}&backurl=" . urlencode(get_url());
        echo "<a class='page-action icon-edit' data-href='{$edit_url}' data-id='pro_{$item["id"]}' title='修改重点项目{$item["id"]}'></a>";

        echo "<a class='icon-remove' onclick='deleteone(this);' delete_id='{$item['id']}' title='删除重点项目{$item["id"]}'></a>";
...
<script type="text/javascript">
    ...
    function deleteone(obj) {
        ...
        if (confirm("确认删除该项(ID:" + id + ")?")) {
            ...
            var url = "<?php echo $url_deleteone;?>";
            $.ajax({
                url: url,
                dataType: "json",
                type: "POST",
