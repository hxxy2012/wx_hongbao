...
<div class="form-inline definewidth m20">
    ...
    <form method="get">
        ...
        <input type="text" name="search_name"
               ...
               placeholder="境内企业名称/联系人/手机"
               value="<?php echo $search_val['name']; ?>"
               style="width:200px;"
        />

        <label for="search_createtime">创建时间:</label>
        ...
        <a id="add_new" class='btn btn-info' data-href='<?php echo site_url("zcq_datamanage/add"); ?>' data-id='351'
           title='添加新的走出去数据'>新增信息</a>
        <a id="btn_import" class='btn btn-info' data-href='<?php echo site_url("zcq_datamanage/import"); ?>' data-id='352'
           title='导入走出去数据'>导入数据</a>
    </form>
</div>
...
<table class="table table-bordered table-hover  m10">
    ...
    foreach ($list as $item) {
        ...
        if ($item['updatetime'] != 0) {
            $time = date("Y-m-d", $item['updatetime']);
        }
        echo "<td>{$time}</td>";
        //修改按钮
        $edit_url = site_url('zcq_datamanage/edit') . "?id={$item["id"]}&backurl=" . urlencode(get_url());
        echo "<td><a class='page-action icon-edit' data-href='{$edit_url}' data-id='userlist_{$item["id"]}' id='open_edit_{$item["uid"]}' title='修改{$item["company"]}'></a></td>";
        echo "</tr>";
        echo "\n";
    }
    ?>
...
<script type="text/javascript">
    ...
    function mutil_delete() {
        ...
        if (confirm("确认删除所选？")) {
            var url = "<?php echo site_url("zcq_datamanage/delete");?>";
            $.ajax({
                url: url,
                dataType: "text",
                type: "GET",
