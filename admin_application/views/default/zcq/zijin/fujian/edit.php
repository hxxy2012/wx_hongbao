    ...
    <form method="POST" name="myform" id="myform" onsubmit="return chkform()">
        ...
        <table class="table table-bordered table-hover  m10">
            <tr>
                ...
                <td>
                    <input type="text" style="width:200px;" maxlength="200" name="title"
                           value="<?php echo $model["title"]; ?>" required/>
                </td>
                <td>
            ...
            <tr>
                ...
                <td>
                    <div id="upload_btn" style="">
                        ...
                        <input type="hidden" name="filepath" id="filepath" value="<?php echo $model["filepath"]; ?>"/>
                    </div>
                    <div id="look_btn" style="display:none;">
                        <input type="button" name="btn_look" onclick="lookfj()" class="button button-success"
                               value="查看"/>
                        <input type="button" onclick="reload()" name="btn_look2" class="button button-danger"
                               value="重新上传"/>
                    </div>
                </td>
                <td>
            ...
            <tr>
                <td colspan="3">
                    <div style="text-align: center;">
                        ...
                        <input class="button" type="button" value="返回" onclick="top.topManager.closePage();"/>
                    </div>
                </td>
...
<script>
    ...
    function lookfj() {
        var baseurl = '<?php echo site_url("zcq_datamanage/readfile")?>';
        var url = baseurl + "?file=" + $("#filepath").val();
        window.open(url);
    }
    function reload() {
    ...
    filepath = "<?php echo $model["filepath"];?>";
    if (filepath != "") {
        $('#upload_btn').css('display', 'none');
        $('#look_btn').css('display', '');
    }
    ...
    //关闭
    var isclose = "<?php echo $close; ?>";
    if (isclose == "yes") {
        top.flushpage('<?php echo $ls;?>');
