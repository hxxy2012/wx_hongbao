...
<div class="form-inline definewidth m20" > 
  活动名称: <input type="text" name="actname" id="actname"class="abc input-default" placeholder="" value="">&nbsp;&nbsp;
  备案类目名: <input type="text" name="lmname" id="lmname"class="abc input-default" placeholder="" value="">&nbsp;&nbsp;
  备案单位: <input type="text" name="badw" id="badw"class="abc input-default" placeholder="" value="">&nbsp;&nbsp;
  <select id="audit_status" name="audit_status">
    <option value="">审核状态</option>
    <option value="0,10,40" <?php if($caozuo==1) echo 'selected';?>>待审核</option>
    <option value="0">未审核</option>
    <option value="10">初审中</option>
    <option value="20">初审通过</option>
    <option value="30">初审不通过</option>
    <option value="40">终审中</option>
    <option value="50">终审通过</option>
    <option value="60">终审不通过</option>
  </select>&nbsp;&nbsp;
...
function common_request(page){
  var url="<?php echo site_url("swj_xxbasp/index");?>?inajax=1";
  var data_ = {
    'page':page,
    'time':<?php echo time();?>,
    'action':'ajax_data',
  ...
  $.ajax({
     ...
     //  async:false,
      ...
      if(msg.resultcode<0){
        ...
        if (list.length <= 0) {
          shtml = '<tr><td style="text-align:center;" colspan="9">暂无信息</td></tr>';
        }
        for(var i in list){
          shtml+='<tr>';
          ...
          shtml+='<td>'+list[i]['actname']+'</td>';
          shtml+='<td>'+list[i]['name']+'</td>';
          shtml+='<td>'+list[i]['company']+'</td>';
          shtml+='<td>'+list[i]['createtime']+'</td>';
          shtml+='<td>'+list[i]['baoxiao_price']+'</td>';
          if (list[i]['audit_status'] == '初审通过') {
            shtml+='<td><a title="查看" href="<?php echo site_url('swj_xxbasp/look?caozuo='.$caozuo);?>&id='+list[i].id+'" class="icon-list-alt"></a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="del_sp('+list[i].id+',this)" class="icon-remove"></a></td>';
          } else {
            shtml+='<td><a title="审核" href="<?php echo site_url('swj_xxbasp/check?caozuo='.$caozuo);?>&id='+list[i].id+'" class="icon-check"></a>&nbsp;&nbsp;<a title="查看" href="<?php echo site_url('swj_xxbasp/look?caozuo='.$caozuo);?>&id='+list[i].id+'" class="icon-list-alt"></a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="del_sp('+list[i].id+',this)" class="icon-remove"></a></td>';
          }
          shtml+='</tr>';
        }
        $("#result_").html(shtml);
...
function del_sp(id, object) {
  if (confirm('您确定要删除该备案吗？')) {
    window.location.href = "<?php echo site_url('swj_xxbasp/del?caozuo='.$caozuo);?>&id=" + id;
  }
}
//批量删除
function pldel() {
  ...
  if (confirm('您确定要删除选中的备案吗？')) {
    window.location.href = "<?php echo site_url('swj_xxbasp/del?caozuo='.$caozuo);?>&id=" + ids;
  }
}
//批量审核
function plshenhe() {
  ...
  if ((audit_status=='30'||audit_status=='60')&&pl_audit_idea=="") {
    alert("请填写审核意见!");
    $('#pl_audit_idea').focus();
    return false;
  }
  ...
  if (confirm('您确定要审核选中的备案吗？')) {
    window.location.href = "<?php echo site_url('swj_xxbasp/shenhe?caozuo='.$caozuo);?>&pl_audit_idea="+pl_audit_idea+"&audit_status="+audit_status+"&id=" + ids;
  }
}
//获取所有选中的复选框的id,以逗号分割
function getIds() {
