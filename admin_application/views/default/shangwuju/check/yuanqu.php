...
<table class="table table-bordered table-hover m10">
    ...
    <tr >
        <td class="tableleft">运营企业对园区的实际投入（万元）</td>
        <td><?php echo $model2["touru"];?></td>
        <td class="tableleft">拟申请资助的金额（元）</td>
        <td><?php echo $model2["zizhu"];?></td>
    </tr>
    <tr>
        <td valign="top" class="tableleft">园区产权情况</td>
        <td colspan="3">
<?php
foreach($yuanqu_chanquan as $v){
	echo "<input type='checkbox' disabled ";
	
	if($v["id"] == $model2["chanquan_id"] ){
		echo " checked ";
	}	
	echo " />";
	echo $v["name"];
	if($model2["chanquan_id"]=="45169"){
		//其他
		echo "(";
		echo $model2["chanquan_beizhu"];
		echo ",年限";
		echo $model2["chanquan_year"];
		if(stripos($model2["chanquan_year"],"年")!==false){
...
?>        
    ...
    <tr>
      <td class="tableleft">园区经营面积（平方米）</td>
      <td><?php echo $model2["jingying_mianji"];?></td>
      <td class="tableleft">其中：用于电子商务相关产业的经营面积（平方米）</td>
      <td ><?php echo $model2["dianshang_mianji"];?></td>
    </tr>
    <tr>
      <td colspan="4" valign="top" style="margin:0px; padding:0px;" ><table class="tabletd" width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td class="tableleft"  style="width:105px;">园区进驻企业数（家）</td>
          <td><?php echo $model2["qiyeshu"];?></td>
          <td class="tableleft">其中电了商务应用企业数（家）</td>
          <td><?php echo $model2["yingyong_qiyeshu"];?></td>
          <td class="tableleft">其中电子商务服务企业数（家）</td>
          <td><?php echo $model2["fuwu_qiyeshu"];?></td>
        </tr>
        <tr>
          <td class="tableleft">年度电子商务营业收入（万元）</td>
