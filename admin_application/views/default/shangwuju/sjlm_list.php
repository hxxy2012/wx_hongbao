...
<div class="form-inline definewidth m20" > 
   上报类目名: <input type="text" name="name" id="name"class="abc input-default" placeholder="" value="">&nbsp;&nbsp; 
  <select id="condition">
    <option value="">可视权限</option>
    <option value="1">公开</option>
    <option value="2">不公开</option>
  </select>&nbsp;&nbsp;
    <button type="submit" class="btn btn-primary" onclick="common_request(1)">查询</button>&nbsp;&nbsp; 
 <a  class="btn btn-primary" id="addnew" href="<?php echo site_url("swj_sjsbgl/add");?>">新增类目<span class="glyphicon glyphicon-plus"></span></a>
</div>
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <th width="64">可视权限</th>
        <th>上报类目名</th>
        <th width="275">上报时段</th>
        <th width="275">审核时段</th>
        <th width="54">操作</th>
    </tr>
    </thead>
  <tbody id="result_">
...
function common_request(page){
  var url="<?php echo site_url("swj_sjsbgl/index");?>?inajax=1";
  var data_ = {
    'page':page,
    'time':<?php echo time();?>,
    'action':'ajax_data',
     ...
     //  async:false,
      ...
      if(msg.resultcode<0){
        ...
        for(var i in list){
          ...
          shtml+='<td>'+list[i]['sb_stime']+'——'+list[i]['sb_etime']+'</td>';
          shtml+='<td>'+list[i]['sh_stime']+'——'+list[i]['sh_etime']+'</td>';
          shtml+='<td><a href="<?php echo site_url('swj_sjsbgl/edit');?>?id='+list[i].id+'" class="icon-edit"></a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="del_lm('+list[i].id+',this)" class="icon-remove"></a></td>';
          shtml+='</tr>';
        }
        $("#result_").html(shtml);
...
function del_lm(id, object) {
  if (confirm('您确定要删除该类目吗？')) {
    window.location.href = "<?php echo site_url('swj_sjsbgl/del');?>?id=" + id;
  }
}
//批量删除
function pldel() {
  ...
  if (confirm('您确定要删除选中的类目吗？')) {
    window.location.href = "<?php echo site_url('swj_sjsbgl/del');?>?id=" + ids;
  }
}
//批量公开
function plgk() {
  ...
  if (confirm('您确定要公开选中的类目吗？')) {
    window.location.href = "<?php echo site_url('swj_sjsbgl/show');?>?isshow=1&id=" + ids;
  }
}
//批量不公开
function plbgk() {
  ...
  if (confirm('您确定不要公开选中的类目吗？')) {
    window.location.href = "<?php echo site_url('swj_sjsbgl/show');?>?isshow=0&id=" + ids;
  }
}
//获取所有选中的复选框的id,以逗号分割
function getIds() {
