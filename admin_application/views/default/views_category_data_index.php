...
<div class="form-inline definewidth m20" >    
    ...
    <button type="button" class="btn btn-danger" onclick="godel()">删除选中</button>    
</div>
<table class="table table-bordered table-hover definewidth m10">
    <thead>
    <tr>
        <th width='10'><input type="checkbox" name="selall" id="selall" onclick="fun_selall()" value=""/></th>
        <th>id</th>
        <th>名称</th>
        <th>类别名称</th>
		<th>seo标题</th>
			...
			if(msg.resultcode<0){
				...
				if(list.length>0){
					for(var i in list){
						...
						shtml+='<tr>';
						shtml+='<td><input type="checkbox" name="id[]" value="'+list[i].id+'"/></td>';
					...
					$("#result_").html("<tr><td colspan='10' align='center'>暂无数据,<a href='javascript:void(0)' onclick=\"goback("+pid+")\">点击返回</a></td></tr>");	
					$("#page_string").html("");	
				}
...
function godel(){
    ...
    $.ajax({
        type: "POST",
        async:false,
        url: "<?php echo site_url("category_data/chksub");?>",
        data: {id:optid },
        dataType: "text",
    ...
    if(isdel){
        $.ajax({
            type: "POST",
            async:false,
            url: "<?php echo site_url("category_data/godel");?>",
            data: {id:optid },
            dataType: "text",
            success: function(data){
