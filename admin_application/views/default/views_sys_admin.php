		   ...
		   success: function(msg){
			...
			if(msg.resultcode<0){
				...
				for(var i in list){
					...
					shtml+='<td><a href="<?php echo site_url('sys_admin/edit');?>?id='+list[i].id+'" class="icon-edit" title="编辑用户'+list[i]['username']+'"></a>';
					if(list[i]['username']!="admin" && isdel){					
						shtml+= '&nbsp;<a class="icon-remove" onclick="del_admin('+list[i].id+')" href="javascript:void(0)"/>';
					};
					shtml+= '';
					shtml+= '</a>';										
					shtml+= '</td>';
...
function del_admin(id){
	var ajax_url = "<?php echo site_url("sys_admin/del");?>?id="+id;	
	var url = "<?php echo site_url("sys_admin/index");?>";
