...
class Category_data extends MY_Controller{
	...
	private function ajax_data(){
		...
		if($list){
			foreach($list as $k=>$v){
				//$list[$k]["checkbox"] = "<input type='checkbox' name='id[]' id='".$v['id']."' value='".$v["id"]."'/>";
		...
		if($list && $list_num){
			...
			foreach($list as $kk=>$vv){
				$list[$kk]['name'] = $vv["name"]."[<b>".(isset($temp[$vv['id']])?$temp[$vv['id']]:'0' )."</b>]";
			}
		}
	...
	function chksub(){
		...
		$optid = $post["id"];
		//检查下级是否有数据，如果有，不准删除
		$list = $this->M_common->querylist("SELECT * FROM {$this->table_}common_category_data WHERE id in($optid)  ");
		$err = "ok";
		foreach($list as $k=>$v){
			$sql = "select * from {$this->table_}common_category_data WHERE pid=".$v["id"];
			if($this->M_common->query_count($sql)>0){
				$err = "编号:".$v["id"]."，名称：".$v["name"]."下级有数据,不能删除";
				break;	
			}
	...
	function godel(){
		$post = $this->input->post();
		$optid = $post["id"];
		$sql = "delete from {$this->table_}common_category_data where id in($optid)";
