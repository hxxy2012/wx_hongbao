...
class Zzb_jjfz_dati_history extends MY_Controller{
	function Zzb_jjfz_dati_history(){
		parent::__construct();
		$this->load->model("M_zzb_jjfz_dati_history","jjfz_history");
	...
	function index(){
		...
		$orderby = array();
		$pagesize = 10;
		$data = $this->jjfz_history->GetInfoList($pageindex,$pagesize,$search,$orderby);
		$data["search_val"] = $search_val;		
		$data["isdel"] = is_super_admin(); //$this->permition_for("zzb_jjfz_dati_history","del");		
	...
	function tongji(){
		...
		$pagesize = 10;
		$data = $this->jjfz_history->GetPaiMing($pageindex,$pagesize,$search);
		$data["search_val"] = $search_val;
		$pm = ($pageindex-1)*$pagesize;
		...
		//print_r($data);
		$this->load->view(__TEMPLET_FOLDER__."/fenzitiku/tongji",$data);
	}	
	...
	function del(){
		...
		$isdel = false;
		if(!is_super_admin()){
			if($this->permition_for('zzb_jjfz_dati_history','del')){
				$isdel = true;
			}
		}
		else{
