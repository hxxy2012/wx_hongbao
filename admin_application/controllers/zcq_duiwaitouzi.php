 ...
 *
 * @property  m_zcq_duiwaitouzi $model 重点项目模型
...
class zcq_duiwaitouzi extends MY_Controller
{
    ...
    private $controller_name = "zcq_duiwaitouzi";
    ...
    public function __construct()
    {
        ...
        $this->data = array();
        $this->views_path = __TEMPLET_FOLDER__ . "/zcq/duiwaitouzi";
    ...
    public function index()
    {
        ...
        //搜索审核状态为1,2,3的
        $search['exp'] = " check_status != '0' ";
        $orderby['check_status'] = 'asc';
        $orderby['id'] = 'desc';

        //结果保存在$this->data中
        $data = $this->model->getList($pageindex, $pagesize, $search, $orderby);
        //搜索变量
        ...
        foreach ($arr as $id) {
            ...
            if ($rows == 0) {
                ...
                if ($model['create_userid'] != 0) {
                    $title = "您好，您的对外投资联系表通过审核，编号为{$id}";
                    $content = "您好，您的对外投资联系表通过审核，编号为{$id}，请到位置：\"会员后台 > 对外投资联系表\" 栏目，查看。";
                    $re = send_zhan_mail($model['create_userid'], '', $title, $content, null, admin_id(), $this->sess["session_id"]);
                }
            }
        }
    ...
    public function notok()
    {
        ...
        foreach ($arr as $id) {
            ...
            if ($rows == 0) {
                ...
                /*发送站内信*/
                if ($model['create_userid'] != 0) {
                    $title = "您好，您的对外投资联系表审核不通过，编号为{$id}";
                    $content = "您好，您的对外投资联系表审核不通过，编号为{$id}，请到位置：\"会员后台 > 对外投资联系表\" 栏目，查看。";
                    $re = send_zhan_mail($model['create_userid'], '', $title, $content, null, admin_id(), $this->sess["session_id"]);
                }
            }
        }
