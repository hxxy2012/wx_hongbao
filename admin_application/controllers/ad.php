...
class Ad extends MY_Controller{
	...
	private function ajax_data(){
		...
		if($list){
			foreach($list as $k=>$v){
				...
				$pic_url ="/".$v['pic'] ;//$this->v_upload_path
				$list[$k]['pic'] = ($v['type'] == 0 )?"<a href='{$pic_url}' target='_blank'><img src='{$pic_url}' width=\"150\" height='90'></a>":'';
	...
	private function doadd(){	
		...
		if($array['affect_num']>=1){
            ...
            //一秒后跳转
            header("refresh:1;url=".site_url("ad/index"));
            echo "<script>
			top.tip_show('添加广告为{$this->name}成功',1,2000);
			</script>";
		}else{
	...
	private function doedit(){
		...
		$this->load->library("common_upload");
		//$pic  = $this->common_upload->upload_path($this->upload_path,"pic") ; 	
		$pic  = $this->common_upload->upload_path_ym_width( $this->upload_path ,"pic" , $allow_type = 'gif|jpg|png|bmp',2500);
		if($pic){
		...
		if($num>=1){
            ...
            header("refresh:1;url=".site_url("ad/index"));
            echo "<script>
			top.tip_show('服务器繁忙，或者你没有修改任何数据',1,1500);
			</script>";
		}
	}
	//删除
