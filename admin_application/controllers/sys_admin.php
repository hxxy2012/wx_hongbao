...
class Sys_admin extends MY_Controller{
	...
	function Sys_admin(){
		...
		$this->load->model('M_common_system_user');
	...
	function index(){
		...
		if($action == 'show'){
			$data["isdel"] = $this->permition_for('sys_admin','del')?'true':'false';
			$this->load->view(__TEMPLET_FOLDER__."/views_sys_admin",$data);
	...
	private function doadd(){
        ...
        $is_receive = verify_id($this->input->get_post("is_receive"));//是否接收邮件
        $email = daddslashes(html_escape(strip_tags($this->input->get_post("email"))));//邮箱
        $website_category = $this->input->get_post("website_category");
                
                
		if($gid<=0 && $super_admin!=1 ){
			showmessage("请选择用户组","sys_admin/add",3,0);
			exit();
		}elseif(abslength($username)<2 || abslength($username)>16){
			showmessage("用户名长度必须在2-16之间","sys_admin/add",3,0);
			exit();
			...
			if(isset($info['super_admin']) && $info['super_admin']){//当前登陆的用户是管理员
				...
				if(empty($info_)){
					...
					exit();
                              ...
                              //  print_r(config_item("super_admin"));exit;
                ...
                //选中的管理栏目
                if($info_["website_category"]!="") {
                    $category = $this->M_common->querylist("select * from website_category where id in(" . $info_["website_category"] . ")");
                }
                else{
                    $category = array();
                }
...
//		}
		...
		if(abslength($username)<2 || abslength($username)>16){
			showmessage("用户名长度必须在2-16之间","sys_admin/edit",3,0,"?id={$id}");
			exit();
		}
		if(!empty($passwd)){
		...
		//写用户的特殊权限到缓存文件
		$sql_edit = "UPDATE `{$this->table_}common_system_user` SET website_category='$website_category',status = '{$status}' {$where} ,username = '{$username}',is_receive='{$is_receive}',
		super_admin = '{$super_admin}',tel='$tel' where id = '{$id}'";
	...
	function del(){
		...
		if(is_numeric($id)){
			//判断ADMIN不能删除
			$model = $this->M_common_system_user->GetModel($id);
			if(is_array($model)){
			...
			$sql_edit = "UPDATE `{$this->table_}common_system_user` SET status = '-1' where id = '$id'";
			//$num = $this->M_common->update_data($sql_edit);
			$this->M_common_system_user->del($id);
			if(true){
